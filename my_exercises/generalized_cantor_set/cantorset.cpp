...
void createCantorSetTikz (std::string filename, unsigned int n_iter, double my_eps, double spacing)
{
	...
	draw_file<<"\\usepackage{amsmath}"<<"\\usepackage{tikz}"<<endl;
	draw_file<<"\\begin{document}"<<endl;
	draw_file<<"\\textbf{Generalized Cantor Set} with $\\epsilon = $"<<my_eps<<endl<<endl<<"\\begin{tikzpicture}"<<endl;
	...
	for (unsigned int c_it = 0; c_it < n_iter; ++c_it)	{
		...
		for (auto it = my_set.subsets.cbegin(); it != my_set.subsets.cend(); ++it)	{
			draw_file<<"\\draw [blue, line width=2.0mm] ("<<factor*(it->getBegin())<<","<<y<<") -- ("<<factor*(it->getEnd())<<","<<y<<");"<<endl;	
		}
		my_set.oneDivision();
		y += spacing;		
...
int main ()
{
	...
	createCantorSetTikz (fname, 10, 0.4, 0.3);
